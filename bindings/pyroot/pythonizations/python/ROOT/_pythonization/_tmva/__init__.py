...
python_classes = [factory]

# list of python functions that are used to pythonize BookMethod function in Tmva
python_tmva_functions = [BookMethod]
...
@pythonization("Tmva", is_prefix=True)
def pythonize_tmva_class(klass, name):
    # Parameters:
    ...
    for func_name in func_names:
        ...
        # if the Tmva
        #  class already has a function with the same name as our
...
def pythonize_tmva_namespace(ns):
    ...
    for python_func in python_tmva_functions:
        ...
        if sys.version_info <= (3, 0):
            # In Python 2 the Tmva is treated like a class and the global
