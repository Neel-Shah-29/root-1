...
std::unique_ptr<ROperator> make_ROperator_Expand(const onnx::NodeProto& nodeproto, const onnx::GraphProto& graphproto, std::unordered_map<std::string, ETensorType>& tensor_type){
   ...
   for (int i = 0; i < 2; ++i) {
      ...
      if (it != tensor_type.end()){
         ...
         if (!isInitializer)
            throw std::runtime_error("TMVA::SOFIE ONNX Parser Expand op has input tensor " + input_name + " but its type is not yet registered");
      }
   }
   ...
   switch(input_type){
   case ETensorType::FLOAT:
      op.reset(new ROperator_Expand<float>(nodeproto.input(0), nodeproto.input(1), nodeproto.output(0)));
      break;
   default:
      throw std::runtime_error("TMVA::SOFIE - Unsupported - Expand Operator does not yet support input type " + std::to_string(static_cast<int>(input_type)));
   }
